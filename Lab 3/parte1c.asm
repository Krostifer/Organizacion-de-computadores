.data
	ingreseNumero: .asciiz "Por favor ingrese el entero al que desea calcular su logaritmo natural: "
	resultado: .asciiz "El resultado del logaritmo natural calculado es: "
	neutroMulDouble: .double 1.0
	dosDouble: .double 2.0
...
.text
	main:
		...
		whileMain:
			...
			#Se asigna 1 en el numerador
			ldc1 $f24, neutroMulDouble
			#Se asigna 2n+1 en el denominador
			mtc1.d $s1, $f26
			cvt.d.w $f26, $f26
			#Se calcula 1/(2n+1)
			jal divD
			...
			jal divD
			#Se almacena el resultado en f4
			add.d $f4, $f0, $f28
			
			#--------------------------------------------------
			#Se calcula la potencia
			#Se asigan la base como argumento
			add.d $f24, $f0, $f4
			#Se asigna 2n+1 como exponente
			add $a2, $zero, $s1
			...
			#Se realiza la multiplicacion entre ambos numeros
			add.d $f26, $f0, $f2
			add.d $f24, $f0, $f4
			jal mulDD
...
#Potencia que recibe como base un double, exponente entero y restorna un doble
	potD:
		#Se guarda el valor de la base
		add.d $f14, $f0, $f24
		#Se guarda el valor del exponente para utilizarlo como contador
